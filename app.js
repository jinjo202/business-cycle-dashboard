...
function updateExportCard() {
    ...
    const baselinePrices = {
        "US": {
            ...
            "AAPL": { name: "Apple Inc.", ticker: "AAPL", base: 313.70, format: (p) => `$${p.toFixed(2)}` },
            "MSFT": { name: "Microsoft Corp.", ticker: "MSFT", base: 511.28, format: (p) => `$${p.toFixed(2)}` },
            "AMZN": { name: "Amazon.com Inc.", ticker: "AMZN", base: 261.25, format: (p) => `$${p.toFixed(2)}` },
            "TSLA": { name: "Tesla Inc.", ticker: "TSLA", base: 366.00, format: (p) => `$${p.toFixed(2)}` },
            "COST": { name: "Costco Wholesale", ticker: "COST", base: 943.16, format: (p) => `$${p.toFixed(2)}` },
            "KO": { name: "Coca-Cola Co.", ticker: "KO", base: 89.07, format: (p) => `$${p.toFixed(2)}` },
            "PG": { name: "Procter & Gamble Co.", ticker: "PG", base: 144.92, format: (p) => `$${p.toFixed(2)}` },
            "MCD": { name: "McDonald's Corp.", ticker: "MCD", base: 265.37, format: (p) => `$${p.toFixed(2)}` },
            "WBA": { name: "Walgreens Boots Alliance", ticker: "WBA", base: 15.40, format: (p) => `$${p.toFixed(2)}` },
            "PARA": { name: "Paramount Global", ticker: "PARA", base: 1.10, format: (p) => `$${p.toFixed(2)}` },
            "INTC": { name: "Intel Corp.", ticker: "INTC", base: 89.70, format: (p) => `$${p.toFixed(2)}` },
            "LMT": { name: "Lockheed Martin Corp.", ticker: "LMT", base: 561.85, format: (p) => `$${p.toFixed(2)}` },
            "GLD": { name: "SPDR Gold Shares", ticker: "GLD", base: 406.61, format: (p) => `$${p.toFixed(2)}` },
            "WMT": { name: "Walmart Inc.", ticker: "WMT", base: 104.39, format: (p) => `$${p.toFixed(2)}` },
            "JPM": { name: "JPMorgan Chase & Co.", ticker: "JPM", base: 355.98, format: (p) => `$${p.toFixed(2)}` },
            "CAT": { name: "Caterpillar Inc.", ticker: "CAT", base: 792.29, format: (p) => `$${p.toFixed(2)}` },
            "XOM": { name: "Exxon Mobil Corp.", ticker: "XOM", base: 159.36, format: (p) => `$${p.toFixed(2)}` },
            "GE": { name: "General Electric Co.", ticker: "GE", base: 334.51, format: (p) => `$${p.toFixed(2)}` },
            "PYPL": { name: "PayPal Holdings Inc.", ticker: "PYPL", base: 53.28, format: (p) => `$${p.toFixed(2)}` },
            "BA": { name: "Boeing Co.", ticker: "BA", base: 206.89, format: (p) => `$${p.toFixed(2)}` },
            "PFE": { name: "Pfizer Inc.", ticker: "PFE", base: 28.40, format: (p) => `$${p.toFixed(2)}` },
            "SQQQ": { name: "ProShares UltraPro Short QQQ", ticker: "SQQQ", base: 38.83, format: (p) => `$${p.toFixed(2)}` },
            "VIX": { name: "iPath Volatility VIX ETN", ticker: "VIX", base: 15.15, format: (p) => `$${p.toFixed(2)}` },
            "XLU": { name: "Utilities Select Sector SPDR", ticker: "XLU", base: 42.18, format: (p) => `$${p.toFixed(2)}` },
            "AMD": { name: "Advanced Micro Devices", ticker: "AMD", base: 468.96, format: (p) => `$${p.toFixed(2)}` },
            "SMCI": { name: "Super Micro Computer", ticker: "SMCI", base: 37.31, format: (p) => `$${p.toFixed(2)}` },
            "TLT": { name: "iShares 20+ Yr Treasury Bond", ticker: "TLT", base: 82.32, format: (p) => `$${p.toFixed(2)}` },
            "BAC": { name: "Bank of America Corp.", ticker: "BAC", base: 62.12, format: (p) => `$${p.toFixed(2)}` },
            "FCX": { name: "Freeport-McMoRan Inc.", ticker: "FCX", base: 75.69, format: (p) => `$${p.toFixed(2)}` }
        },
        "KR": {
...
function calculateSecondaryIndicators(macro, season, activeRegion, rate, eps, m2) {
    ...
    const mcapSp500Total = 81.64 * (1 + ytdSp500 / 100);
    const mcapKospiTotal = 5577.86 * (1 + ytdKospi / 100);
    const mcapKosdaqTotal = 549.65 * (1 + ytdKosdaq / 100);
    ...
    // US S&P 500 Top 10 Companies
        ...
        { name: "Apple Inc.", ticker: "AAPL", baseMcap: 4.58, betaX: 3.5, betaY: 1.8, baseOffset: 9.0, price: 299.09, fwdPE: 31.15, eps_trail: 8.26, eps_0q: 1.89, eps_1q: 2.01, eps_0y: 8.76, eps_1y: 9.67, ret_1d: 0.7, ret_1w: 0.9, ret_1m: -3.0, last_updated: "2026-06-24 00:16" },
    ...
    // KOSPI Top 10 Companies
            const krTop10Defs = [
        { name: "삼성전자", ticker: "005930", baseMcap: 1707.31, betaX: 5.5, betaY: 3.0, baseOffset: 8.0, price: 310000.0, fwdPE: 5.34, eps_trail: 0, eps_0q: 11061.49, eps_1q: 13483.35, eps_0y: 45425.67, eps_1y: 58035.6, ret_1d: 0.0, ret_1w: 0.0, ret_1m: 0.0, last_updated: "2026-06-23 15:30" },
        { name: "SK하이닉스", ticker: "000660", baseMcap: 1188.30, betaX: 8.5, betaY: 5.0, baseOffset: 24.0, price: 2555000.0, fwdPE: 6.18, eps_trail: 0, eps_0q: 71064.06, eps_1q: 88259.91, eps_0y: 305490.4, eps_1y: 413579.4, ret_1d: 0.0, ret_1w: 0.0, ret_1m: 0.0, last_updated: "2026-06-23 15:30" },
        { name: "현대차", ticker: "005380", baseMcap: 105.65, betaX: 3.5, betaY: 1.5, baseOffset: 14.0, price: 511000.0, fwdPE: 10.36, eps_trail: 0, eps_0q: 12370.13, eps_1q: 12614.77, eps_0y: 44708.26, eps_1y: 49327.39, ret_1d: 0.0, ret_1w: 0.0, ret_1m: 0.0, last_updated: "2026-06-23 15:30" },
        { name: "LG에너지솔루션", ticker: "373220", baseMcap: 88.45, betaX: 6.0, betaY: 3.5, baseOffset: 5.0, price: 362000.0, fwdPE: 42.11, eps_trail: 0, eps_0q: 878.92, eps_1q: 2508.59, eps_0y: 1979.04, eps_1y: 8596.31, ret_1d: 0.0, ret_1w: 0.0, ret_1m: 0.0, last_updated: "2026-06-23 15:30" },
        { name: "삼성물산", ticker: "028260", baseMcap: 61.61, betaX: 2.0, betaY: 1.5, baseOffset: 5.0, price: 455000.0, fwdPE: 25.83, eps_trail: 0, eps_0q: 3136.25, eps_1q: 3472.02, eps_0y: 15864.73, eps_1y: 17613.21, ret_1d: 0.0, ret_1w: 0.0, ret_1m: 0.0, last_updated: "2026-06-23 15:30" },
        ...
        { name: "KB금융", ticker: "105560", baseMcap: 60.85, betaX: 2.5, betaY: 1.2, baseOffset: 18.0, price: 153700.0, fwdPE: 8.13, eps_trail: 0, eps_0q: 4578.9, eps_1q: 4985.44, eps_0y: 17648.36, eps_1y: 18906.17, ret_1d: 0.0, ret_1w: 0.0, ret_1m: 0.0, last_updated: "2026-06-23 15:30" },
        { name: "기아", ticker: "000270", baseMcap: 50.91, betaX: 3.8, betaY: 1.6, baseOffset: 15.0, price: 137400.0, fwdPE: 5.82, eps_trail: 0, eps_0q: 6129.74, eps_1q: 5699.84, eps_0y: 21668.62, eps_1y: 23614.46, ret_1d: 0.0, ret_1w: 0.0, ret_1m: 0.0, last_updated: "2026-06-23 15:30" },
        { name: "신한지주", ticker: "055550", baseMcap: 51.12, betaX: 2.2, betaY: 1.1, baseOffset: 14.0, price: 97700.0, fwdPE: 7.58, eps_trail: 0, eps_0q: 3301.84, eps_1q: 3203.13, eps_0y: 11800.98, eps_1y: 12897.58, ret_1d: 0.0, ret_1w: 0.0, ret_1m: 0.0, last_updated: "2026-06-23 15:30" },
        { name: "현대모비스", ticker: "012330", baseMcap: 40.44, betaX: 2.5, betaY: 1.2, baseOffset: 6.0, price: 510000.0, fwdPE: 9.61, eps_trail: 0, eps_0q: 13246.62, eps_1q: 13516.65, eps_0y: 47320.18, eps_1y: 53062.31, ret_1d: 0.0, ret_1w: 0.0, ret_1m: 0.0, last_updated: "2026-06-23 15:30" }
    ];
