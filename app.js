...
function updateExportCard() {
    ...
    const baselinePrices = {
        "US": {
            "NVDA": { name: "NVIDIA Corp.", ticker: "NVDA", base: 230.36, format: (p) => `$${p.toFixed(2)}` },
            "AAPL": { name: "Apple Inc.", ticker: "AAPL", base: 319.97, format: (p) => `$${p.toFixed(2)}` },
            "MSFT": { name: "Microsoft Corp.", ticker: "MSFT", base: 499.70, format: (p) => `$${p.toFixed(2)}` },
            "AMZN": { name: "Amazon.com Inc.", ticker: "AMZN", base: 258.51, format: (p) => `$${p.toFixed(2)}` },
            "TSLA": { name: "Tesla Inc.", ticker: "TSLA", base: 354.08, format: (p) => `$${p.toFixed(2)}` },
            "COST": { name: "Costco Wholesale", ticker: "COST", base: 915.74, format: (p) => `$${p.toFixed(2)}` },
            "KO": { name: "Coca-Cola Co.", ticker: "KO", base: 88.07, format: (p) => `$${p.toFixed(2)}` },
            "PG": { name: "Procter & Gamble Co.", ticker: "PG", base: 146.44, format: (p) => `$${p.toFixed(2)}` },
            "MCD": { name: "McDonald's Corp.", ticker: "MCD", base: 255.69, format: (p) => `$${p.toFixed(2)}` },
            ...
            "INTC": { name: "Intel Corp.", ticker: "INTC", base: 95.80, format: (p) => `$${p.toFixed(2)}` },
            "LMT": { name: "Lockheed Martin Corp.", ticker: "LMT", base: 525.28, format: (p) => `$${p.toFixed(2)}` },
            "GLD": { name: "SPDR Gold Shares", ticker: "GLD", base: 406.77, format: (p) => `$${p.toFixed(2)}` },
            "WMT": { name: "Walmart Inc.", ticker: "WMT", base: 107.14, format: (p) => `$${p.toFixed(2)}` },
            "JPM": { name: "JPMorgan Chase & Co.", ticker: "JPM", base: 358.64, format: (p) => `$${p.toFixed(2)}` },
            "CAT": { name: "Caterpillar Inc.", ticker: "CAT", base: 813.94, format: (p) => `$${p.toFixed(2)}` },
            "XOM": { name: "Exxon Mobil Corp.", ticker: "XOM", base: 159.47, format: (p) => `$${p.toFixed(2)}` },
            "GE": { name: "General Electric Co.", ticker: "GE", base: 337.12, format: (p) => `$${p.toFixed(2)}` },
            "PYPL": { name: "PayPal Holdings Inc.", ticker: "PYPL", base: 54.96, format: (p) => `$${p.toFixed(2)}` },
            "BA": { name: "Boeing Co.", ticker: "BA", base: 212.25, format: (p) => `$${p.toFixed(2)}` },
            "PFE": { name: "Pfizer Inc.", ticker: "PFE", base: 28.45, format: (p) => `$${p.toFixed(2)}` },
            "SQQQ": { name: "ProShares UltraPro Short QQQ", ticker: "SQQQ", base: 38.18, format: (p) => `$${p.toFixed(2)}` },
            "VIX": { name: "iPath Volatility VIX ETN", ticker: "VIX", base: 14.53, format: (p) => `$${p.toFixed(2)}` },
            "XLU": { name: "Utilities Select Sector SPDR", ticker: "XLU", base: 43.08, format: (p) => `$${p.toFixed(2)}` },
            "AMD": { name: "Advanced Micro Devices", ticker: "AMD", base: 477.57, format: (p) => `$${p.toFixed(2)}` },
            "SMCI": { name: "Super Micro Computer", ticker: "SMCI", base: 39.59, format: (p) => `$${p.toFixed(2)}` },
            "TLT": { name: "iShares 20+ Yr Treasury Bond", ticker: "TLT", base: 82.21, format: (p) => `$${p.toFixed(2)}` },
            "BAC": { name: "Bank of America Corp.", ticker: "BAC", base: 62.68, format: (p) => `$${p.toFixed(2)}` },
            "FCX": { name: "Freeport-McMoRan Inc.", ticker: "FCX", base: 72.73, format: (p) => `$${p.toFixed(2)}` }
        },
        "KR": {
...
function calculateSecondaryIndicators(macro, season, activeRegion, rate, eps, m2) {
    ...
    const mcapSp500Total = 82.10 * (1 + ytdSp500 / 100);
    const mcapKospiTotal = 5469.24 * (1 + ytdKospi / 100);
    const mcapKosdaqTotal = 535.95 * (1 + ytdKosdaq / 100);

    // US S&P 500 Top 10 Companies
            const usTop10Defs = [
        { name: "NVIDIA Corp.", ticker: "NVDA", baseMcap: 5.56, betaX: 8.0, betaY: 6.0, baseOffset: 22.0, price: 201.07, fwdPE: 15.8, eps_trail: 6.53, eps_0q: 2.08, eps_1q: 2.35, eps_0y: 8.96, eps_1y: 12.73, ret_1d: -3.62, ret_1w: -5.34, ret_1m: -6.29, last_updated: "2026-06-24 00:16" },
        ...
        { name: "Amazon.com Inc.", ticker: "AMZN", baseMcap: 2.79, betaX: 4.5, betaY: 2.2, baseOffset: 11.0, price: 234.39, fwdPE: 23.72, eps_trail: 7.36, eps_0q: 1.81, eps_1q: 1.91, eps_0y: 8.66, eps_1y: 9.88, ret_1d: 0.69, ret_1w: -4.73, ret_1m: -11.65, last_updated: "2026-06-24 00:16" },
