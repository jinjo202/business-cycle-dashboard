...
function updateExportCard() {
    ...
    const baselinePrices = {
        "US": {
            "NVDA": { name: "NVIDIA Corp.", ticker: "NVDA", base: 223.46, format: (p) => `$${p.toFixed(2)}` },
            "AAPL": { name: "Apple Inc.", ticker: "AAPL", base: 336.66, format: (p) => `$${p.toFixed(2)}` },
            "MSFT": { name: "Microsoft Corp.", ticker: "MSFT", base: 514.31, format: (p) => `$${p.toFixed(2)}` },
            "AMZN": { name: "Amazon.com Inc.", ticker: "AMZN", base: 248.10, format: (p) => `$${p.toFixed(2)}` },
            "TSLA": { name: "Tesla Inc.", ticker: "TSLA", base: 368.92, format: (p) => `$${p.toFixed(2)}` },
            "COST": { name: "Costco Wholesale", ticker: "COST", base: 919.30, format: (p) => `$${p.toFixed(2)}` },
            "KO": { name: "Coca-Cola Co.", ticker: "KO", base: 87.84, format: (p) => `$${p.toFixed(2)}` },
            "PG": { name: "Procter & Gamble Co.", ticker: "PG", base: 145.74, format: (p) => `$${p.toFixed(2)}` },
            "MCD": { name: "McDonald's Corp.", ticker: "MCD", base: 237.07, format: (p) => `$${p.toFixed(2)}` },
            "WBA": { name: "Walgreens Boots Alliance", ticker: "WBA", base: 15.40, format: (p) => `$${p.toFixed(2)}` },
            "PARA": { name: "Paramount Global", ticker: "PARA", base: 0.72, format: (p) => `$${p.toFixed(2)}` },
            "INTC": { name: "Intel Corp.", ticker: "INTC", base: 124.69, format: (p) => `$${p.toFixed(2)}` },
            "LMT": { name: "Lockheed Martin Corp.", ticker: "LMT", base: 522.01, format: (p) => `$${p.toFixed(2)}` },
            "GLD": { name: "SPDR Gold Shares", ticker: "GLD", base: 391.33, format: (p) => `$${p.toFixed(2)}` },
            "WMT": { name: "Walmart Inc.", ticker: "WMT", base: 107.32, format: (p) => `$${p.toFixed(2)}` },
            "JPM": { name: "JPMorgan Chase & Co.", ticker: "JPM", base: 340.01, format: (p) => `$${p.toFixed(2)}` },
            "CAT": { name: "Caterpillar Inc.", ticker: "CAT", base: 809.20, format: (p) => `$${p.toFixed(2)}` },
            "XOM": { name: "Exxon Mobil Corp.", ticker: "XOM", base: 160.91, format: (p) => `$${p.toFixed(2)}` },
            "GE": { name: "General Electric Co.", ticker: "GE", base: 322.38, format: (p) => `$${p.toFixed(2)}` },
            "PYPL": { name: "PayPal Holdings Inc.", ticker: "PYPL", base: 55.08, format: (p) => `$${p.toFixed(2)}` },
            "BA": { name: "Boeing Co.", ticker: "BA", base: 198.00, format: (p) => `$${p.toFixed(2)}` },
            "PFE": { name: "Pfizer Inc.", ticker: "PFE", base: 28.22, format: (p) => `$${p.toFixed(2)}` },
            "SQQQ": { name: "ProShares UltraPro Short QQQ", ticker: "SQQQ", base: 34.19, format: (p) => `$${p.toFixed(2)}` },
            "VIX": { name: "iPath Volatility VIX ETN", ticker: "VIX", base: 15.56, format: (p) => `$${p.toFixed(2)}` },
            "XLU": { name: "Utilities Select Sector SPDR", ticker: "XLU", base: 39.37, format: (p) => `$${p.toFixed(2)}` },
            "AMD": { name: "Advanced Micro Devices", ticker: "AMD", base: 630.21, format: (p) => `$${p.toFixed(2)}` },
            "SMCI": { name: "Super Micro Computer", ticker: "SMCI", base: 42.78, format: (p) => `$${p.toFixed(2)}` },
            "TLT": { name: "iShares 20+ Yr Treasury Bond", ticker: "TLT", base: 78.90, format: (p) => `$${p.toFixed(2)}` },
            "BAC": { name: "Bank of America Corp.", ticker: "BAC", base: 56.05, format: (p) => `$${p.toFixed(2)}` },
            "FCX": { name: "Freeport-McMoRan Inc.", ticker: "FCX", base: 71.91, format: (p) => `$${p.toFixed(2)}` }
        },
        "KR": {
...
function calculateSecondaryIndicators(macro, season, activeRegion, rate, eps, m2) {
    ...
    const mcapSp500Total = 81.97 * (1 + ytdSp500 / 100);
    const mcapKospiTotal = 5791.24 * (1 + ytdKospi / 100);
    const mcapKosdaqTotal = 556.36 * (1 + ytdKosdaq / 100);

    // US S&P 500 Top 10 Companies
            const usTop10Defs = [
        { name: "NVIDIA Corp.", ticker: "NVDA", baseMcap: 5.40, betaX: 8.0, betaY: 6.0, baseOffset: 22.0, price: 201.07, fwdPE: 15.8, eps_trail: 6.53, eps_0q: 2.08, eps_1q: 2.35, eps_0y: 8.96, eps_1y: 12.73, ret_1d: -3.62, ret_1w: -5.34, ret_1m: -6.29, last_updated: "2026-06-24 00:16" },
        ...
        { name: "Alphabet Inc.", ticker: "GOOGL", baseMcap: 4.19, betaX: 3.8, betaY: 1.9, baseOffset: 8.0, price: 346.5, fwdPE: 23.81, eps_trail: 13.11, eps_0q: 2.87, eps_1q: 3.0, eps_0y: 14.22, eps_1y: 14.51, ret_1d: -0.89, ret_1w: -6.17, ret_1m: -10.83, last_updated: "2026-06-24 00:16" },
        { name: "Microsoft Corp.", ticker: "MSFT", baseMcap: 3.82, betaX: 4.0, betaY: 2.0, baseOffset: 10.0, price: 374.87, fwdPE: 19.38, eps_trail: 16.79, eps_0q: 4.24, eps_1q: 4.62, eps_0y: 16.84, eps_1y: 19.35, ret_1d: 2.05, ret_1w: -6.23, ret_1m: -9.89, last_updated: "2026-06-24 00:16" },
        { name: "Amazon.com Inc.", ticker: "AMZN", baseMcap: 2.68, betaX: 4.5, betaY: 2.2, baseOffset: 11.0, price: 234.39, fwdPE: 23.72, eps_trail: 7.36, eps_0q: 1.81, eps_1q: 1.91, eps_0y: 8.66, eps_1y: 9.88, ret_1d: 0.69, ret_1w: -4.73, ret_1m: -11.65, last_updated: "2026-06-24 00:16" },
        { name: "Broadcom Inc.", ticker: "AVGO", baseMcap: 1.67, betaX: 4.8, betaY: 2.5, baseOffset: 13.0, price: 380.28, fwdPE: 19.62, eps_trail: 6.0, eps_0q: 3.24, eps_1q: 3.87, eps_0y: 11.62, eps_1y: 19.39, ret_1d: -3.02, ret_1w: -3.31, ret_1m: -9.75, last_updated: "2026-06-24 00:16" },
        ...
        { name: "Meta Platforms", ticker: "META", baseMcap: 1.91, betaX: 5.2, betaY: 3.0, baseOffset: 12.0, price: 565.12, fwdPE: 15.59, eps_trail: 27.52, eps_0q: 7.2, eps_1q: 7.04, eps_0y: 32.83, eps_1y: 34.92, ret_1d: 0.24, ret_1w: -4.76, ret_1m: -7.61, last_updated: "2026-06-24 00:16" },
